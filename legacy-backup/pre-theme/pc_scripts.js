...
async function listScores(songId) {
  ...
  if (!res.ok) return [];
  return cleanScoreFiles(await res.json());
}

function scoreUrl(path) {
  return `${STORAGE_URL}/object/public/scores/${encodeURIComponent(path).replaceAll('%2F','/')}`;
}
function isScoreImageFile(f){
  const n=String(f?.name||'').toLowerCase();
  return !!n && !n.endsWith('/') && /\.(png|jpe?g|webp|gif)$/i.test(n);
}
function cleanScoreFiles(files){
  return (Array.isArray(files)?files:[]).filter(isScoreImageFile);
}
function brokenScoreImage(img){
  try{
    const page=img.closest('.viewer-page,.score-preview,.viewer-body,.score-grid');
    if(page && page.classList.contains('viewer-page')){
      page.classList.add('empty');
      page.innerHTML='악보 파일 확인 필요<small>삭제되었거나 경로가 맞지 않는 페이지입니다.</small>';
    }else{
...
async function syncR2Praise(songIds) {
  const ids = Array.isArray(songIds) ? songIds.filter(Boolean) : (songIds ? [songIds] : []);
  ...
  async function postJson(payload) {
    ...
    if (!res.ok || data.ok === false) throw new Error(data.error || data.message || `R2 반영 실패 (${res.status})`);
    return data;
  }
  ...
  try {
    if (ids.length) {
      try {
        await postJson({songIds: ids, force: true});
        return true;
      } catch (targetError) {
        const oneByOneErrors = [];
        for (const id of ids) {
          try {
            await postJson({songId: id, force: true});
          } catch (e) {
            oneByOneErrors.push(e.message || String(e));
          }
        }
        if (!oneByOneErrors.length) return true;
      }
    }
    ...
    let guard = 0;
    while (guard < 100) {
      const data = await postJson({force: true, batch: true, offset, limit: 50});
      if (data.done) return true;
      const processed = Number(data.processedCount || data.limit || 0);
      const nextOffset = Number(data.nextOffset || 0);
      if (nextOffset > offset) offset = nextOffset;
      else if (processed > 0) offset += processed;
      else return true;
      guard++;
    }
    throw new Error('R2 전체 반영이 너무 오래 걸려 중단됐어요');
  } catch (e) {
    if (typeof dbg === 'function') dbg('R2 반영 실패: ' + (e.message || e));
...
async function openViewer(id, name) {
  ...
  try {
    ...
    document.getElementById('viewer-body').innerHTML = files.map(f=>`
      <img src="${scoreUrl(id+'/'+f.name)}" alt="악보" loading="lazy" onerror="brokenScoreImage(this)">`).join('');
  } catch(e) {
...
async function saveSong() {
  ...
  try {
    if (editId) {
      const updated = await saveSongRecord('PATCH', `songs?id=eq.${editId}`, { name, status, count, seasons, key, keys: formKeys, bpm, bpm_min:bpmMin, bpm_max:bpmMax, music_url, updated_at: new Date().toISOString() });
      ...
      const created = await saveSongRecord('POST', 'songs', { name, status, count, seasons, key, keys: formKeys, bpm, bpm_min:bpmMin, bpm_max:bpmMax, music_url });
...
async function loadRightScorePreview(song){
  ...
  try{
    ...
    box.innerHTML=files.slice(0,2).map(f=>`<img src="${scoreUrl(song.id+'/'+f.name)}" alt="${escHtml(song.name)} 악보" loading="lazy" onerror="brokenScoreImage(this)">`).join('') + (files.length>2?`<div class="viewer-empty-panel" style="width:100%"><button class="ab" onclick="openViewer('${jsStr(song.id)}','${jsStr(song.name)}')">전체 ${files.length}페이지 보기</button></div>`:'');
