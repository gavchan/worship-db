...
async function load(){
  ...
  try{
    ...
    const rawIds=Array.isArray(h.song_ids)?h.song_ids:[];
    ...
    document.getElementById('content').innerHTML=names.map((name,i)=>{const id=rawIds[i]||'';const s=id?(songMap[id]||{}):{};const key=s.key||'';const bpm=s.bpm||'';const memo=s.memo||'';return `<div class="song"><div class="song-head"><div class="num">${i+1}</div><div class="name">${esc(name)}</div></div><div class="meta">${key?`<span class="pill">Key ${esc(key)}</span>`:''}${bpm?`<span class="pill">BPM ${esc(bpm)}</span>`:''}<span class="pill">${s.has_score?'악보 있음':'악보 확인 필요'}</span>${s.music_url?'<span class="pill">영상 있음</span>':''}</div>${memo?`<div class="memo">${esc(memo)}</div>`:''}</div>`}).join('')||'<div class="empty">등록된 곡이 없습니다.</div>';
